...
class ProjectEular {
    ...
    01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48`;
        let testArray = sampleData.split("\n").map((l) => l
            .split(" ")
            .filter(Boolean)
            .map((x) => +x));
    ...
    twelve() {
        ...
        let x = 1;
        while (findFactors(triangleNumber(x)).length <= 100)
            x++;
    ...
    fourteen() {
        console.log("I can't Understand");
        const evenN = function (n) {
            return n / 2;
        };
        const oddN = function (n) {
            return 3 * n + 1;
        };
    }
    fifteen() {
